...
def test_no_template_strings_after_param_substitution():
    ...
    combos = [
        ...
        ("eval_existing", {"input_eval_dataset_source": "existing"}),
        ("eval_synthetic", {"input_eval_dataset_source": "synthetic"}),
        ("eval_production_traces", {"input_eval_dataset_source": "production_traces"}),
...
def test_eval_output():
    ...
    @test("eval files always generated — gates.yml")
    def _(): assert file_exists(gen(), "src/agents/default/eval/gates.yml")

    @test("eval files always generated — evaluate_agent.py")
    def _(): assert file_exists(gen(), "src/agents/default/eval/evaluate_agent.py")
    ...
    def _():
        content = read_file(gen(input_eval_dataset_source="synthetic"), "src/agents/default/eval/create_dataset.py")
        assert "generate_evals_df" in content

    # --- Dataset source: manual ---
    @test("manual — create_dataset.py has inline examples")
    def _():
        content = read_file(gen(input_eval_dataset_source="manual"), "src/agents/default/eval/create_dataset.py")
        assert "eval_examples" in content
        assert "generate_evals_df" not in content

    # --- Dataset source: production_traces ---
    @test("production_traces — create_dataset.py uses search_traces")
    def _():
        content = read_file(gen(input_eval_dataset_source="production_traces"), "src/agents/default/eval/create_dataset.py")
        assert "search_traces" in content
        assert "eval_candidate" in content

    # --- Dataset source: existing ---
    @test("existing — create_dataset.py skipped")
    def _():
        p = gen(input_eval_dataset_source="existing")
        assert not file_exists(p, "src/agents/default/eval/create_dataset.py")

    @test("existing — other eval files remain")
    def _():
        p = gen(input_eval_dataset_source="existing")
        assert file_exists(p, "src/agents/default/eval/gates.yml")
        assert file_exists(p, "src/agents/default/eval/evaluate_agent.py")
        assert file_exists(p, "src/agents/default/eval/utils.py")

    # --- Gates derived from components ---
    @test("gates.yml — safety always present")
    def _():
        content = read_file(gen(), "src/agents/default/eval/gates.yml")
        assert "safety" in content
        assert "fluency" in content

    @test("gates.yml — VS enabled adds relevance/groundedness")
    def _():
        content = read_file(gen(input_use_vector_search="yes"), "src/agents/default/eval/gates.yml")
        assert "relevance" in content
        assert "groundedness" in content

    @test("gates.yml — VS disabled excludes relevance/groundedness")
    def _():
        content = read_file(gen(input_use_vector_search="no"), "src/agents/default/eval/gates.yml")
        assert "relevance" not in content
        assert "groundedness" not in content

    @test("gates.yml — UC enabled adds tool_call_correctness")
    def _():
        content = read_file(gen(input_use_uc_functions="yes"), "src/agents/default/eval/gates.yml")
        assert "tool_call_correctness" in content

    @test("gates.yml — UC disabled excludes tool_call_correctness")
    def _():
        content = read_file(gen(input_use_uc_functions="no"), "src/agents/default/eval/gates.yml")
        assert "tool_call_correctness" not in content

    # --- evaluate_agent.py imports from utils ---
    @test("evaluate_agent.py imports from utils")
    def _():
        content = read_file(gen(), "src/agents/default/eval/evaluate_agent.py")
...
def test_component_combinations():
    ...
    combos = [
        ...
        ("UC + eval existing", {"input_use_uc_functions": "yes", "input_eval_dataset_source": "existing"}),
